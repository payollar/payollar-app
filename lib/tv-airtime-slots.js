 ...
 * @returns {{ id: string, range: string, label: string, weeklyGhs: number, booked: boolean }[]}
 */
export function mapRateCardToAirtimeSlots(rateCard, options = {}) {
  ...
  for (const section of rateCard.sections || []) {
    for (const table of section.tables || []) {
      ...
      const rateCol =
        columns.find((c) => getColumnRole(c) === "rate_ad_type") ||
        columns.find((c) => c.dataType === "CURRENCY");
      ...
      for (const row of table.rows || []) {
        ...
        let weeklyGhs = 0;
        if (rateCol) {
          const v = parseFloat(getCell(row, rateCol.id));
          if (!isNaN(v)) weeklyGhs = v;
        } else {
