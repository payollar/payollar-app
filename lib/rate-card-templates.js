...
const TIME_CLASS_OPTIONS = ["Premium", "M1", "M2", "M3", "M4"];

function buildAdTypeTimeClassTemplate(adTypeId, adTypeLabel) {
  return {
    key: `ad_type_${adTypeId.replace(/-/g, "_")}_time_class`,
    label: `${adTypeLabel} (by Time Class)`,
    description: `Rates for ${adTypeLabel} per time class. Used in the period calculator when clients select this ad type and a time class.`,
    defaultTitle: "Rates by Time Class",
    columns: [
      {
        name: "Ad Type",
        dataType: "TEXT",
        ...
        config: { role: "ad_type_identifier" },
      },
      {
      ...
      },
      {
        ...
        config: { role: "rate_ad_type" },
      },
    ],
    rows: TIME_CLASS_OPTIONS.map((tc) => ({
      "Ad Type": adTypeId,
      "Time Class / Segment": tc,
      "Rate per 30s (₵)": "",
    })),
  };
}

function buildAdTypeTimeClassTemplates() {
  const allAdTypes = [
    { id: "tvc", label: "TVC" },
    { id: "lpm", label: "LPM" },
    ...
    { id: "produced-spot", label: "Produced Spot" },
    { id: "live-read", label: "Live Read" },
  ];
  const seen = new Set();
  return allAdTypes
    .filter((a) => {
    ...
    .map((a) => buildAdTypeTimeClassTemplate(a.id, a.label));
}
...
export const SMART_RATE_CARD_TEMPLATES = [
  ...
  // Per-ad-type templates: each ad type gets its own section with time-class-linked rates
  ...buildAdTypeTimeClassTemplates(),
];
...
/** Get template key for an ad type's time-class rates section */
export function getAdTypeTimeClassTemplateKey(adTypeId) {
  return `ad_type_${String(adTypeId).replace(/-/g, "_")}_time_class`;
}

/** Ad types that have time-class templates, by media type */
export const TV_AD_TYPE_IDS = ["tvc", "lpm", "sponsorship", "product-placement", "documentary"];
export const RADIO_AD_TYPE_IDS = ["jingle", "produced-spot", "live-read", "lpm", "sponsorship"];
